...
import (
	...
	"github.com/archplatform/script-service/internal/handler"
	"github.com/archplatform/script-service/internal/server"
...
func main() {
	...
	scriptEngine := engine.NewEngine(
		cfg.Engine.PythonExecutable,
		cfg.Engine.MaxExecutionTime,
		cfg.Engine.MaxMemoryMB,
		cfg.Engine.SandboxEnabled,
	)

	// Create handler
	h := handler.NewHandler(postgresStorage, scriptEngine)
	...
	httpServer := &http.Server{
		Addr:    ":" + getEnv("HTTP_PORT", "8085"),
		Handler: router,
	}

	// Start gRPC server in background
	grpcServer := server.NewScriptServer(postgresStorage, scriptEngine, cfg)
	go func() {
